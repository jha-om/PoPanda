...
export const POST = async (req: NextRequest) => {
    try {
        ...
        // *how quota system works in SaaS, is implemented below
        ...
        // why quotaLimit should never be hard-coded? 
        // because it's very difficult to maintain, what if these hard coded are also present somewhere else, 
        // might be very difficult to debug if issue comes;
        // therefore we should always create a config.ts/js file into root folder;
        ...
        if (!category) {
            return NextResponse.json({
                message: `You don't have a category name ${validationResult.category}`
            }, { status: 404 })
        }
        ...
        try {
            await discord.sentEmbed(dmChannel.id, eventData);
    
            await db.event.update({
                where: { id: event.id },
                data: { delieveryStatus: "DELIVERED" }
            })
