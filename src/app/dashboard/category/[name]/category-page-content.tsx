...
export const CategoryPageContent = ({ hasEvents: initialHasEvents, category }: CategoryPageContentProps) => {
    ...
    const { data, isFetching } = useQuery({
        ...
        // on switching the windows like going back, or forth, it should not fetch things always until new data is appended;
        ...
        data.events.forEach((event) => {
            ...
            Object.entries(event.fields as object).forEach(([field, value]) => {
                if (typeof value === "number") {
                    ...
                    // the day that event was created by user;
                    // isafter compares two dates =>
                    // let's say today's date = 10/08/2025;
                    // and client made that event happen on => 27/07/2025
                    // then this date and today's date lies in different weeks' and we are taking sunday as our start day for the week;

                    if (isAfter(eventDate, weekStart) || eventDate.getTime() === weekStart.getTime()) {
                        sums[field].thisWeek += value;
                    }

                    // same logic goes with the month;
